...
df <- pd.read_csv("../data/processed/all_experiments.csv")
...
models[['all']] <- glmer(focus_chosen ~ 1 + (1|uid/study), family = 'binomial', data = df)
...
stargazer(models, 
          ...
          apply.coef = exp,  # Exponentiate coefficients to show odds ratios
          t.auto = F, 
          p.auto = F, 
          report = "vct*",  # Report coefficient, t-value, and standard error
          add.lines = variance_rows,  # Add both variance components, 
          title="Mixed effect logistic results from efficacy studies. Participants chose between Plurals or non-Plurals output. The outcome variable is choosing Plurals. The fixed effect intercept is the odds ratios of choosing Plurals output, accounting for random effects. Models 1-3 have a random intercept for participants. Model 4 collapses across studies, with a random intercept for participants nested within studies.",
          type='latex', 
          label="summary", 
